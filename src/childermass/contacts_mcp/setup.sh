#!/usr/bin/env bash
#
# Childermass Contacts MCP Server – Setup Script
#
# Usage:
...
echo "=== Childermass Contacts MCP – Setup ==="
...
CREDS_FILE="$HOUSTON_DIR/contacts-credentials.json"
if [ ! -f "$CREDS_FILE" ]; then
    echo ""
    echo "⚠  OAuth credentials not found at $CREDS_FILE"
    echo ""
    echo "   To set up People API access:"
    echo "   1. Go to https://console.cloud.google.com/"
    echo "   2. Create a project and enable People API"
...
from childermass.contacts_mcp import __version__
from childermass.contacts_mcp.security import SecurityError, rate_limiter, audit_log
from childermass.contacts_mcp.auth import list_authenticated_accounts
from childermass.contacts_mcp.client import get_people_service
from childermass.contacts_mcp.server import mcp
...
if [ -f "$CREDS_FILE" ]; then
    echo "  1. Authenticate:"
    echo "     PYTHONPATH=$PROJECT_ROOT/src python3 -m childermass.contacts_mcp.auth --account=your@email.com"
    echo ""
    echo "  2. Enable in OpenCode (.opencode/opencode.json):"
    echo '     "contacts": { "enabled": true }'
    echo ""
    echo "  3. Run tests:"
