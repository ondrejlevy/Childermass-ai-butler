...
Handles OAuth2 flow for Gmail API access with secure token storage.
...
Run with: python -m childermass.gmail_mcp.auth --account=your@email.com
"""
...
# Gmail API scopes - minimum required
SCOPES = [
...
DEFAULT_CREDENTIALS_PATH = DEFAULT_TOKEN_DIR / "gmail-credentials.json"

# Keyring service name
KEYRING_SERVICE = "childermass-gmail-mcp"
...
def _get_legacy_token_path(account: str | None = None) -> Path:
    ...
    if account:
        return DEFAULT_TOKEN_DIR / f"gmail-tokens-{account}.json"
    return Path(
        os.getenv(
            "GMAIL_TOKEN_PATH",
            str(DEFAULT_TOKEN_DIR / "gmail-tokens.json"),
        )
    )
...
def get_credentials_path() -> Path:
    ...
    return Path(os.getenv("GMAIL_CREDENTIALS_PATH", str(DEFAULT_CREDENTIALS_PATH)))
...
def list_authenticated_accounts() -> list[str]:
    ...
    if DEFAULT_TOKEN_DIR.exists():
        for token_file in DEFAULT_TOKEN_DIR.glob("gmail-tokens-*.json"):
            email = token_file.stem.replace("gmail-tokens-", "")
            accounts.add(email)
...
def get_authenticated_credentials(account: str | None = None) -> Credentials:
    ...
    msg = (
        ...
        f"  python -m childermass.gmail_mcp.auth --account={account or 'your@email.com'}"
    )
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        description="Authenticate Gmail accounts for Childermass MCP",
        epilog=(
            "Examples:\n"
