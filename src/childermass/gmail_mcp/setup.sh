#!/usr/bin/env bash
#
# Childermass Gmail MCP Server – Setup Script
#
# Usage:
...
echo "=== Childermass Gmail MCP – Setup ==="
...
CREDS_FILE="$HOUSTON_DIR/gmail-credentials.json"
if [ ! -f "$CREDS_FILE" ]; then
    echo ""
    echo "⚠  OAuth credentials not found at $CREDS_FILE"
    echo ""
    echo "   To set up Gmail API access:"
...
from childermass.gmail_mcp import __version__
from childermass.gmail_mcp.security import SecurityError, rate_limiter, audit_log
from childermass.gmail_mcp.auth import list_authenticated_accounts
from childermass.gmail_mcp.client import get_gmail_service
from childermass.gmail_mcp.server import mcp
...
if [ -f "$CREDS_FILE" ]; then
    echo "  1. Authenticate:"
    echo "     PYTHONPATH=$PROJECT_ROOT/src python3 -m childermass.gmail_mcp.auth --account=your@email.com"
    echo ""
    echo "  2. Enable in OpenCode (.opencode/opencode.json):"
    echo '     "gmail": { "enabled": true }'
    echo ""
    echo "  3. Run tests:"
