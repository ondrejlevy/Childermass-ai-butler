...
Handles OAuth2 flow for Google Places API (New) access with secure token
storage.  Tokens are stored in the system keyring (macOS Keychain / Linux
Secret Service).  Falls back to encrypted file storage if keyring is
unavailable.

Run with: python -m childermass.places_mcp.auth --account=your@email.com
"""
...
# used for user-scoped access and quota attribution.  The scope below
# covers all Places (New) REST endpoints.
SCOPES = [
...
DEFAULT_CREDENTIALS_PATH = DEFAULT_TOKEN_DIR / "places-credentials.json"

# Keyring service name
KEYRING_SERVICE = "childermass-places-mcp"
...
def _is_keyring_available() -> bool:
    ...
    try:
        ...
        test_key = "__childermass_keyring_test__"
...
def _get_legacy_token_path(account: str | None = None) -> Path:
    ...
    if account:
        return DEFAULT_TOKEN_DIR / f"places-tokens-{account}.json"
    return Path(
        os.getenv(
            "PLACES_TOKEN_PATH",
            str(DEFAULT_TOKEN_DIR / "places-tokens.json"),
        )
    )
...
def list_authenticated_accounts() -> list[str]:
    """
    List all authenticated Places API accounts.

    Returns:
        List of account identifiers that have valid tokens.
    """
    accounts: set[str] = set()

    # Check file-based tokens
    if DEFAULT_TOKEN_DIR.exists():
        for token_file in DEFAULT_TOKEN_DIR.glob("places-tokens-*.json"):
            email = token_file.stem.replace("places-tokens-", "")
            accounts.add(email)
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        description="Authenticate Google Places API accounts for Childermass MCP",
        epilog=(
            "Examples:\n"
            "  python -m childermass.places_mcp.auth --account=user@gmail.com\n"
            "  python -m childermass.places_mcp.auth --list\n"
            "  python -m childermass.places_mcp.auth --migrate-keyring\n"
            "  python -m childermass.places_mcp.auth --revoke user@gmail.com"
        ),
        formatter_class=argparse.RawDescriptionHelpFormatter,
    )
    parser.add_argument(
        "--account",
        help="Email address / account to authenticate",
        metavar="EMAIL",
    )
    parser.add_argument(
        "--list",
