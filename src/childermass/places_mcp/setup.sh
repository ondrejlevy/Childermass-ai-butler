#!/usr/bin/env bash
#
# Childermass Places MCP Server – Setup Script
#
# Usage:
...
echo "=== Childermass Places MCP – Setup ==="
...
CREDS_FILE="$HOUSTON_DIR/places-credentials.json"
if [ ! -f "$CREDS_FILE" ]; then
    echo ""
    echo "⚠  OAuth credentials not found at $CREDS_FILE"
    echo ""
    echo "   To set up Places API access:"
    echo "   1. Go to https://console.cloud.google.com/"
    echo "   2. Create a project (or use existing Childermass project)"
    echo "   3. Enable 'Places API (New)' in APIs & Services"
    echo "   4. Create OAuth 2.0 credentials (Desktop app type)"
    echo "   5. Download the JSON and save to:"
    echo "      $CREDS_FILE"
    echo ""
    echo "   Note: The Places API (New) requires the"
    echo "   'cloud-platform' OAuth scope."
    echo ""
else
...
from childermass.places_mcp import __version__
from childermass.places_mcp.security import SecurityError, rate_limiter, audit_log
from childermass.places_mcp.auth import list_authenticated_accounts
from childermass.places_mcp.client import text_search, nearby_search
from childermass.places_mcp.server import mcp
...
if [ -f "$CREDS_FILE" ]; then
    echo "  1. Authenticate:"
    echo "     PYTHONPATH=$PROJECT_ROOT/src python3 -m childermass.places_mcp.auth --account=your@email.com"
    echo ""
    echo "  2. Enable in OpenCode (.opencode/opencode.json):"
    echo '     "places": { "enabled": true }'
    echo ""
    echo "  3. Run tests:"
