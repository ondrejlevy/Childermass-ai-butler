#!/usr/bin/env bash
#
# Childermass UniFi Network MCP Server – Setup Script
#
# Usage:
...
echo "=== Childermass UniFi Network MCP – Setup ==="
...
CONFIG_FILE="$HOUSTON_DIR/network-config.json"
if [ -f "$CONFIG_FILE" ]; then
    chmod 600 "$CONFIG_FILE"
    echo "✓ Console configuration: $CONFIG_FILE"
else
    echo ""
    echo "⚠  Console configuration not found at $CONFIG_FILE"
    echo ""
    echo "   Run interactive setup:"
    echo "   source venv/bin/activate"
    echo "   PYTHONPATH=src python -m childermass.network_mcp.auth --setup"
    echo ""
fi
...
from childermass.network_mcp import __version__
from childermass.network_mcp.security import SecurityError, rate_limiter, audit_log
from childermass.network_mcp.auth import load_config, get_console_url
from childermass.network_mcp.server import mcp
...
echo ""
echo "Next steps:"
echo "  1. Configure console connection:"
echo "     PYTHONPATH=src python -m childermass.network_mcp.auth --setup"
echo ""
echo "  2. Test connectivity:"
echo "     PYTHONPATH=src python -m childermass.network_mcp.auth --test"
echo ""
echo "  3. Enable in OpenCode (.opencode/opencode.json):"
echo '     "network": {'
echo '       "type": "stdio",'
echo '       "command": "python",'
echo '       "args": ["-m", "childermass.network_mcp.server"],'
echo '       "env": {"PYTHONPATH": "src"},'
echo '       "enabled": true'
echo '     }'
echo ""
echo "  4. Run tests:"
echo "     PYTHONPATH=src pytest src/childermass/network_mcp/tests/ -v"
