...
Handles OAuth2 flow for Google Tasks API access with secure token storage.
...
Uses SEPARATE credentials file from Gmail and Calendar (tasks-credentials.json).

Run with: python -m childermass.tasks_mcp.auth --account=your@email.com
"""
...
DEFAULT_CREDENTIALS_PATH = DEFAULT_TOKEN_DIR / "tasks-credentials.json"

# Keyring service name (separate from Gmail and Calendar)
KEYRING_SERVICE = "childermass-tasks-mcp"
...
def _is_keyring_available() -> bool:
    ...
    try:
        ...
        test_key = "__childermass_tasks_keyring_test__"
...
def _get_legacy_token_path(account: str | None = None) -> Path:
    ...
    if account:
        return DEFAULT_TOKEN_DIR / f"tasks-tokens-{account}.json"
    return Path(
        os.getenv(
            "TASKS_TOKEN_PATH",
            str(DEFAULT_TOKEN_DIR / "tasks-tokens.json"),
        )
    )
...
def get_credentials_path() -> Path:
    ...
    return Path(
        os.getenv("TASKS_CREDENTIALS_PATH", str(DEFAULT_CREDENTIALS_PATH))
    )


def list_authenticated_accounts() -> list[str]:
    """
    List all authenticated Tasks accounts.
    ...
    if DEFAULT_TOKEN_DIR.exists():
        for token_file in DEFAULT_TOKEN_DIR.glob("tasks-tokens-*.json"):
            email = token_file.stem.replace("tasks-tokens-", "")
            accounts.add(email)
...
def get_authenticated_credentials(account: str | None = None) -> Credentials:
    ...
    raise RuntimeError(
        ...
        f"  python -m childermass.tasks_mcp.auth --account={account or 'your@email.com'}"
    )
...
def authenticate(account: str | None = None) -> None:
    ...
    if not credentials_path.exists():
        ...
        print("\nTo set up Google Tasks API access:")
        print("1. Go to https://console.cloud.google.com/")
        print("2. Create a project and enable Google Tasks API")
    ...
    print("You can now use the Tasks MCP server.\n")
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        description="Authenticate Google Tasks accounts for Childermass MCP",
        epilog=(
            "Examples:\n"
            "  python -m childermass.tasks_mcp.auth --account=user@gmail.com\n"
            "  python -m childermass.tasks_mcp.auth --list\n"
            "  python -m childermass.tasks_mcp.auth --migrate-keyring\n"
            "  python -m childermass.tasks_mcp.auth --revoke user@gmail.com"
        ),
