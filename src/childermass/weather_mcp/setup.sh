#!/usr/bin/env bash
#
# Childermass Weather MCP Server – Setup Script
#
# Usage:
#   cd /Users/ondrej.levy/Agents/Home
#   ./src/childermass/weather_mcp/setup.sh
#
set -euo pipefail
...
echo "=== Childermass Weather MCP – Setup ==="
...
from childermass.weather_mcp import __version__
from childermass.weather_mcp.security import SecurityError, rate_limiter
from childermass.weather_mcp.auth import get_api_key, AuthenticationError
from childermass.weather_mcp.client import get_client
from childermass.weather_mcp.server import mcp
print(f'✓ All modules loaded (v{__version__})')
"

# 6. Check API key or prompt for it
echo ""
API_KEY_FILE="$HOUSTON_DIR/weather_api_key"
API_KEY_EXISTS=false

# Check if API key is already configured
if PYTHONPATH="$PROJECT_ROOT/src" python3 -m childermass.weather_mcp.auth --verify 2>/dev/null; then
    API_KEY_EXISTS=true
fi
...
if [ "$API_KEY_EXISTS" = false ]; then
    ...
    echo ""
    echo "  You need an OpenWeatherMap API key to use this MCP server."
    echo "  Get a free key at: https://openweathermap.org/api"
    echo ""
    echo "  Free tier includes:"
    echo "    • 1,000 API calls per day"
    echo "    • Current weather data"
    echo "    • 5-day / 3-hour forecast"
    echo "    • Weather alerts"
    echo ""
    ...
    if [ -n "$API_KEY" ]; then
        ...
        PYTHONPATH="$PROJECT_ROOT/src" python3 -m childermass.weather_mcp.auth --set-api-key "$API_KEY"
    else
        echo ""
        echo "⚠  Skipped API key configuration"
        echo "   Configure later with:"
        echo "   PYTHONPATH=$PROJECT_ROOT/src python3 -m childermass.weather_mcp.auth --set-api-key YOUR_KEY"
    fi
else
...
echo '           "args": ["-m", "childermass.weather_mcp.server"],'
echo "           \"cwd\": \"$SCRIPT_DIR\","
echo '           "env": {'
...
echo "     PYTHONPATH=$PROJECT_ROOT/src python3 $SCRIPT_DIR/test_setup.py"
echo ""
echo "  3. Run tests:"
