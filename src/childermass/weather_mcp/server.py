...
@mcp.tool()
def weather_get_current(location: str, units: str = "metric") -> dict:
...
@mcp.tool()
def weather_get_forecast(location: str, days: int = 5, units: str = "metric") -> dict:
...
@mcp.tool()
def weather_get_daily(location: str, days: int = 5, units: str = "metric") -> dict:
...
@mcp.tool()
def weather_check_rain(location: str, hours: int = 12, units: str = "metric") -> dict:
    ...
    try:
        ...
        for forecast in forecasts:
            if forecast.timestamp <= cutoff and (forecast.pop > 0 or forecast.rain_3h):
                rain_periods.append(
                    {
...
@mcp.tool()
def weather_check_temperature_range(location: str, days: int = 3, units: str = "metric") -> dict:
...
def weather_compare_locations(
    locations: list[str], metric: str = "temperature", units: str = "metric"
) -> dict:
        ...
        for location in locations:
            try:
                ...
                if metric == "temperature":
                    ...
                    data["description"] = (
                        weather.conditions[0].description if weather.conditions else "Unknown"
                    )
                else:
                    return {
                        "error": f"Invalid metric: {metric}. Use: temperature, humidity, wind, conditions"
                    }

                results.append(data)
            except Exception as e:
                results.append({"location": location, "error": sanitize_error_message(e)})
...
def weather_is_good_for_activity(
    location: str, activity: str, when: str = "now", units: str = "metric"
) -> dict:
    ...
    try:
        ...
        if when == "now":
            ...
            wind = (
                sum(f.wind_speed for f in day_forecasts) / len(day_forecasts)
                if day_forecasts
                else 0.0
            )
            humidity = (
                sum(f.humidity for f in day_forecasts) / len(day_forecasts)
                if day_forecasts
                else 0.0
            )
...
def weather_search_best_conditions(
    locations: list[str], preferences: dict, units: str = "metric"
) -> dict:
            ...
            try:
                ...
                if "temp_min" in preferences:
                    if temp >= preferences["temp_min"]:
                        score += 1
                        reasons.append(
                            f"Temperature {temp}° meets minimum {preferences['temp_min']}°"
                        )
                    else:
                        reasons.append(
                            f"Temperature {temp}° below minimum {preferences['temp_min']}°"
                        )
                ...
                if "temp_max" in preferences:
                    if temp <= preferences["temp_max"]:
                        score += 1
                        reasons.append(
                            f"Temperature {temp}° within maximum {preferences['temp_max']}°"
                        )
                    else:
                        reasons.append(
                            f"Temperature {temp}° exceeds maximum {preferences['temp_max']}°"
                        )

                if preferences.get("no_rain"):
                    conditions = weather.conditions[0].main if weather.conditions else "Unknown"
                ...
                if "max_wind" in preferences:
                    if weather.wind_speed <= preferences["max_wind"]:
                        score += 1
                        reasons.append(
                            f"Wind {weather.wind_speed} within limit {preferences['max_wind']}"
                        )
                    else:
                        reasons.append(
                            f"Wind {weather.wind_speed} exceeds limit {preferences['max_wind']}"
                        )
...
@mcp.tool()
def weather_get_sunrise_sunset(location: str, date: str = "today") -> dict:
