...
Handles OAuth2 flow for Google Calendar API access with secure token storage.
...
Uses SEPARATE credentials file from Gmail (calendar-credentials.json).

Run with: python -m childermass.calendar_mcp.auth --account=your@email.com
"""
...
DEFAULT_CREDENTIALS_PATH = DEFAULT_TOKEN_DIR / "calendar-credentials.json"

# Keyring service name (separate from Gmail)
KEYRING_SERVICE = "childermass-calendar-mcp"
...
def _is_keyring_available() -> bool:
    ...
    try:
        ...
        test_key = "__childermass_calendar_keyring_test__"
...
def _get_legacy_token_path(account: str | None = None) -> Path:
    ...
    if account:
        return DEFAULT_TOKEN_DIR / f"calendar-tokens-{account}.json"
    return Path(
        os.getenv(
            "CALENDAR_TOKEN_PATH",
            str(DEFAULT_TOKEN_DIR / "calendar-tokens.json"),
        )
    )
...
def get_credentials_path() -> Path:
    ...
    return Path(
        os.getenv("CALENDAR_CREDENTIALS_PATH", str(DEFAULT_CREDENTIALS_PATH))
    )
...
def list_authenticated_accounts() -> list[str]:
    ...
    if DEFAULT_TOKEN_DIR.exists():
        for token_file in DEFAULT_TOKEN_DIR.glob("calendar-tokens-*.json"):
            email = token_file.stem.replace("calendar-tokens-", "")
            accounts.add(email)
...
def get_authenticated_credentials(account: str | None = None) -> Credentials:
    ...
    raise RuntimeError(
        ...
        f"  python -m childermass.calendar_mcp.auth --account={account or 'your@email.com'}"
    )
...
def authenticate(account: str | None = None) -> None:
    ...
    print("You can now use the Calendar MCP server.\n")
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        description="Authenticate Google Calendar accounts for Childermass MCP",
        epilog=(
            "Examples:\n"
